...
class control_rod {
    constructor(x) {
        this.lowered = false;
        this.x = x;
        this.thickness = CONTROL_ROD_THICKNESS;
    }

    absorb_neutrons(neutron_list) {
        ...
        for (let neutron_index = 0; neutron_index < neutron_list.length; neutron_index++) {
            ...
            let collision = (neutron.x >= this.x && neutron.x <= (this.x + this.thickness));
...
for (let i = 0; i < rows; i++) {
    if ((i + 1) % 8 == 0) {
        control_rods.push(new control_rod(
            i * WATER_CELL_SIZE + WATER_CELL_SIZE / 2
        ))
    }
}
...
const update_simulation = async () => {
    ...
    for (let rod_index = 0; rod_index < control_rods.length; rod_index++) {
        let rod = control_rods[rod_index];
        if (rod === undefined) { continue; }

        CONTEXT.fillStyle = "rgb(46, 46, 46)";
        CONTEXT.fillRect(rod.x - rod.thickness / 2, 0, rod.thickness, BOARD.height);
        rod.lowered = true;
        rod.absorb_neutrons(neutrons);
    }
